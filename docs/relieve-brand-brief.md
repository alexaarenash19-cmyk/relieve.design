...
- [ ] **"¿Para quién es esta pieza?" (sección 10 punto 4) — ya existía, no se duplicó.** El fieldset de tamaño en `Product.jsx` ya usa "¿Para quién es esta pieza — para ti, o para presumirla?" como legend (del brief Rayo X, jul 2026 — el eje tamaño→contexto social/emocional). Es prácticamente el mismo texto que pide esta sección; no se agregó como pregunta standalone separada para no duplicar el mismo mensaje dos veces en la misma página. Revisar con Ale si debe vivir como elemento propio en vez de legend de un fieldset.
- [ ] **No implementado en esta fase — sección 10 (arquitectura completa de página de producto).** Reordenamiento de layout con prioridad horizontal (imagen domina sobre texto), carrusel reordenado (2-3 fotos → foto de contexto → video de unboxing) y soporte de video en `photography.js`/`PhotoCarousel.jsx` (decisión 10) quedan pendientes — es un rediseño visual real que necesita verificación visual en vivo (Vercel/Chrome), no solo edición de código; no se intentó a ciegas mientras los deploys de Vercel siguen bloqueados (ver bitácora de la sesión). `ProductPanel.jsx` no se tocó (decisión 6, correcto — se mantiene separado).
