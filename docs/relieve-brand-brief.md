...
- [x] **Numeración de pieza/edición — resuelta (8 ago 2026).** Ale decidió: secuencia global por pedido pagado — una sola sequence de Postgres (`piece_number_seq`) asignada como `DEFAULT nextval(...)` de `order_items.piece_number`, en el momento del pago (única vía real de inserción en `order_items` es el webhook de Stripe en `checkout.session.completed`, así que el DEFAULT logra "asignar en el pago" sin tocar `api/webhooks/stripe.js`). El mismo número sirve como "pieza N.º" y "edición N.º" — no son dos contadores separados. Migración `supabase/migrations/20260808010001_piece_numbering.sql`, ya aplicada contra la base real de Supabase. `OrderStatus.jsx` ahora integra `FichaTecnica` por ítem del pedido (antes mostraba `place_id` crudo — bug preexistente, corregido de paso).

---
## 19. Fase 4 — Método Relieve, Curva de Nivel, trust bar, integración de ficha técnica (registro de ejecución)

- [x] **Trust bar (sección 12).** Nuevo `src/components/TrustBar.jsx` — no existía ningún componente de trust bar en el repo (verificado por búsqueda de código, el copy solo vivía en este doc). Ticker CSS puro (contenido duplicado + `translateX(-50%)`, sin salto visible), `fixed top-0`, respeta `prefers-reduced-motion`. Copy exacto de la sección 12 incluyendo "Hecho a mano en México". `Nav.jsx` se corrió de `top-0` a `top-7` (28px, la altura del trust bar) para no quedar tapado debajo. Archivos: `src/components/TrustBar.jsx` (nuevo), `src/index.css` (keyframes), `src/App.jsx` (montaje), `src/components/Nav.jsx` (reposicionamiento).
- [x] **Nav — "Colecciones · Método · Reseñas" (sección 5/11, decisión 7).** `src/components/Nav.jsx`: `Buscar`/`Regalar` se quitan del nav superior (rutas `/buscar`/`/regalar` intactas, solo dejan de estar enlazadas ahí). `Reseñas` apunta a `/colecciones#resenas` (ancla ya usada por `Gallery.jsx`).
- [x] **Página `/metodo-relieve` (sección 5).** Nuevo `src/pages/MetodoRelieve.jsx` con los 4 pasos exactos del brief (Captura, Curaduría a mano, Modelado y relieve, Ensamble en parota). Ruta agregada en `src/App.jsx` (lazy, mismo patrón que el resto de páginas).
- [x] **Link "Cómo se hizo esta pieza" (sección 5/10 punto 9).** `src/pages/Product.jsx`: link directo a `/metodo-relieve` justo debajo del campo de placa grabada (Personalización) — único punto de entrada a esa página desde la ficha de producto, sin contenido inline adicional.
- [x] **Curva de Nivel (sección 8, decisión 9).** Tabla nueva `curva_de_nivel` (email + timestamp, sin `place_id` — es a nivel sitio) vía `supabase/migrations/20260807010001_curva_de_nivel.sql`, con RLS habilitado (mismo lockdown por defecto que el resto de tablas públicas). Endpoint `POST /api/curva-de-nivel` agregado como resource `curva_de_nivel` dentro de `api/catalog.js` (no un archivo de función separado) — sigue el patrón ya establecido en ese archivo para `waitlist` (mismo motivo: mantener margen bajo el límite de 12 funciones del plan Hobby de Vercel), reutilizando `checkRateLimit`/`sendError` como en `api/reviews.js`. Rewrite agregado en `vercel.json`. Componente `src/components/CurvaDeNivel.jsx` (nuevo) con el copy exacto de la sección 8 y CTA "Quiero ser parte" (nunca "Suscribirse"). Nota: la migración solo agrega el `.sql` al repo — aplicarla contra Supabase es un paso de deploy/ops aparte, mismo patrón que las migraciones de historias de lugar y `places.series`.
- [x] **Estructura de Home (sección 11).** `src/pages/Home.jsx`: `<CurvaDeNivel />` se monta justo después de `<Gallery />` en las dos ramas que la renderizan (post-hero y `prefers-reduced-motion`). `src/App.jsx`: el `Footer` ya no se suprime en `/` (revierte explícitamente la petición anterior, decisión 11) — como `Footer` vive fuera de `<Routes>`, cae naturalmente debajo de lo que `Home.jsx` pinte al final, sin acoplar `App.jsx` a una página específica.
- [x] **Integración de `FichaTecnica` en `Product.jsx` (decisión 6, cierra el pendiente de la Fase 3/§18).** Reemplaza el bloque "Especificaciones" (un `<dl>` ad-hoc, `fullSpecs`) por `<FichaTecnica>`, pasando la selección real de tamaño/marco/color (nunca un valor fijo) y `series`/`country` reales de `place` — `country` se agregó a la proyección de `getPlaceDetail` en `api/catalog.js` (ya existía en la tabla, default `'MX'`, nunca se seleccionaba). `collectionName` usa el mismo mapeo que el propio ejemplo de `FichaTecnica.jsx` ("Juego"/"Ciudades del Mundo") — no hay un campo de colección expuesto por la API todavía. `pieceNumber`/`editionNumber` se pasan en `null`: `FichaTecnica.jsx` se ajustó para que esos props sean opcionales y omita esas líneas (en vez de renderizar "undefined") mientras la numeración siga sin resolver.
- [x] **Limpieza de `specs` (sección 6).** `src/pages/Product.jsx`: se quitan `SKU` y `Coordenadas` de la franja compacta de specs junto al título (`SKU` era una cadena generada, no un dato real; `Coordenadas` está explícitamente en la lista de campos eliminados). `Tipo`/`Medidas`/`Altitud` se quedan — no están en esa lista.
- [x] **"¿Para quién es esta pieza?" (sección 10 punto 4) — se mantiene como legend, no se duplica.** Confirmado al cerrar la sección 10 completa (§20): el fieldset de tamaño sigue usando "¿Para quién es esta pieza — para ti, o para presumirla?" como legend, ahora reposicionado justo debajo del gancho emocional (punto 3), en vez de vivir como texto standalone separado — sería el mismo mensaje repetido dos veces en la misma página.
- [x] **Sección 10 (arquitectura completa de página de producto) — implementada, ver §20.**

---

## 20. Fase 5 — Arquitectura de página de producto (sección 10) + video en el carrusel (registro de ejecución)

- [x] **Layout horizontal-priority.** `src/pages/Product.jsx`: grid `minmax(0,11fr) minmax(0,9fr)` (antes `md:grid-cols-2`, 50/50) — el carrusel domina visualmente sobre el texto. Nota de implementación: un valor arbitrario de Tailwind (`grid-cols-[11fr_9fr]`) *sin* el `minmax(0,...)` explícito no alcanza — sin él, el track del carrusel vuelve al tamaño mínimo automático del navegador (respeta el min-content de la foto) y el grid computado terminó siendo 928px/128px en vez de ~55/45%, rompiendo los chips de tamaño (`rounded-full` con texto largo forzado a envolver). `grid-cols-2` de Tailwind ya trae `minmax(0,1fr)` por default; con un valor arbitrario hay que escribirlo a mano. Verificado con `getComputedStyle` en el preview de Vercel antes de mergear.
- [x] **Reordenamiento de contenido al orden fijo de la sección 10.** Nombre → gancho emocional → "¿para quién es esta pieza?" → historia del lugar → selectores (marco/color/orientación) + ficha técnica (con la selección real reflejada) → "en una frase, ¿por qué este lugar?" → personalización → link "Cómo se hizo esta pieza" → precio+CTA → tiempos → cómo llega → trust. El gancho emocional es la primera oración de `place.story` (`place.story.split(/(?<=\.)\s+/)`) — las 6 historias reales ya se redactaron como gancho+cuerpo+cierre en un solo bloque (sección 7), así que no se inventa un campo nuevo en la API.
- [x] **Limpieza del bloque "Especificaciones" duplicado.** El `<dl>` de Tipo/Medidas junto al título quedó redundante con `FichaTecnica` (que ya cubre Dimensiones/Marco/Color) tras la integración de la Fase 4 — se retira. Altitud (`elevation_m`, dato real no cubierto por `FichaTecnica`) se conserva, ahora como subtítulo bajo el nombre.
- [x] **Video en el carrusel (decisión 10).** `src/lib/photography.js`: el glob de piezas incluye ahora `mp4`/`webm`/`mov`; `piecePhotos()` devuelve `{url, type}[]` en vez de `string[]`. `PhotoCarousel.jsx`/`Lightbox.jsx` distinguen `<video>` de `<img>` por `type` (loop+mudo en el carrusel, `controls`+sonido en el lightbox). Los videos se ordenan siempre al final del carrusel (fotos hero → foto de contexto → video), sin inventar una convención de nombre de archivo. Ningún video real existe todavía — el glob no matchea nada hasta que se suba uno, mismo comportamiento que las fotos hoy.
- [x] **`ProductPanel.jsx` adaptado sin cambios de arquitectura (decisión 6).** Se ajustó a la nueva forma `{url, type}` de `piecePhotos()` (antes trataba cada entrada como un string plano), filtrando los videos — sigue siendo un preview de solo fotos, sin reproducir video.
- [x] **Verificado en vivo.** Merge directo a `main` (PR #153) + verificación visual en `relieve.design` sobre producción real, en una pieza de pared (`ciudad-de-mexico`, con marco/color/orientación) y en el puzzle (`nevado-de-toluca`, sin esos selectores) — ambos casos renderizan correctamente.
